<template>
  <div class="app-container">
    ...
    <el-table :data="tableList" v-loading="listLoading" border element-loading-text="拼命加载中" style="width: 100%;">
      <el-table-column prop="jsonStr.missDoctor.name" label="名称">
        <template slot-scope="scope">
          {{scope.row.jsonStr.missDoctor.name}}
        </template>
      </el-table-column>
      <el-table-column prop="jsonStr.missDoctor.sex" label="性别">
        <template slot-scope="scope">
          <span v-if="scope.row.jsonStr.missDoctor.sex==='0'">男</span>
          <span v-else>女</span>
        </template>
      </el-table-column>
      <el-table-column prop="jsonStr.missDoctor.origin" label="籍贯">
        <template slot-scope="scope">
          {{scope.row.jsonStr.missDoctor.origin}}
        </template>
      </el-table-column>
      <el-table-column prop="jsonStr.missDoctor.nation" label="民族">
        <template slot-scope="scope">
          {{scope.row.jsonStr.missDoctor.nation}}
        </template>
      </el-table-column>
    ...
    <create-cooperation-dialog :visible.sync="isShowCreateVisible" :row-data="curRowData"
                          :cur-task-type="curTaskType" @refreshList="fetchData"></create-cooperation-dialog>
