<template>
  <div class="app-container">
    ...
    <el-table :data="tableList" v-loading="listLoading" border element-loading-text="拼命加载中" style="width: 100%;">
      <el-table-column prop="jsonStr.missWestern.epidemiology" label="流行病学">
        <template slot-scope="scope">
          {{scope.row.jsonStr.missWestern.epidemiology}}
        </template>
      </el-table-column>
      <el-table-column prop="jsonStr.missWestern.tRelatedDiseases" label="相关疾病">
        <template slot-scope="scope">
          {{scope.row.jsonStr.missWestern.tRelatedDiseases}}
        </template>
      </el-table-column>
    ...
    <create-western-dialog  :visible.sync="isShowCreateVisible" :row-data="curRowData"
...
<script>
  import { getWestDiseaseList, doCreateDisBasics } from '../../api/task'
  import enumerate from '../../store/modules/enumerate'

  import createWesternDialog from './dialog/createWesternDialog'
  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        // this.listQuery
        getWestDiseaseList(params).then(response => {
          const limit = 10
