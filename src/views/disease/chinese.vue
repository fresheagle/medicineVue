<template>
  <div class="app-container">
    ...
    <el-table :data="tableList" v-loading="listLoading" border element-loading-text="拼命加载中" style="width: 100%;">
      <el-table-column prop="jsonStr.missChineseDisease.pathogeny" label="病因">
        <template slot-scope="scope">
          {{scope.row.jsonStr.missChineseDisease.pathogeny}}
        </template>
      </el-table-column>
      <el-table-column prop="jsonStr.missChineseDisease.attentionMatter" label="注意事项">
        <template slot-scope="scope">
          {{scope.row.jsonStr.missChineseDisease.attentionMatter}}
        </template>
      </el-table-column>
...
<script>
  import { getChineseDiseaseList, doCreateDisBasics } from '../../api/task'
